...
You must verify the domain you'll send from (`alll.nsh.one`) so emails don't land in spam.

### 2.1 Add the domain

1. Go to **Brevo → Settings → Senders & IP → Domains**
2. Click **Add a domain**
3. Enter your domain (`alll.nsh.one`)
...
|---|---|---|---|
| TXT | `@` or `alll.nsh.one` | `brevo-code:xxxxxxxx` | Domain ownership |
...
3. Name it `alll-production` (create a separate one for staging)
...
```
Type: MX
Name: inbound.alll.nsh.one   (or replies.alll.nsh.one)
Value: inbound.brevo.com
Priority: 10
TTL: 3600
```

All emails sent to `*@inbound.alll.nsh.one` will be forwarded to your webhook.
...
```
Reply-To: replies+{thread_id}@inbound.alll.nsh.one
```
...
3. Set:
   - **Domain / Email**: `inbound.alll.nsh.one` (or the address from step 5)
   - **Destination URL**: `https://alll.nsh.one/webhooks/brevo/inbound`
...
```json
{
  ...
  "To": "replies+abc123@inbound.alll.nsh.one",
...
# --- Reply-To routing ---
BREVO_INBOUND_DOMAIN=inbound.alll.nsh.one
```
...
kubectl patch secret alll-secrets -n alll-production \
  ...
  -p='[
    ...
    {"op":"add","path":"/data/BREVO_INBOUND_DOMAIN","value":"'$(echo -n "inbound.alll.nsh.one" | base64)'"},
...
| Sender Email | e.g. `ivan@alll.nsh.one` — must be a verified sender in Brevo |
...
> If all agents share the same domain (e.g. `@alll.nsh.one`) and you've verified the domain in step 2, individual addresses on that domain are automatically trusted — no per-sender verification needed.
...
2. Add URL: `https://alll.nsh.one/webhooks/brevo/events`
