    ...
    protected function getHeaderActions(): array
    {
        return [
            ...
            Action::make('download_pdf')
                ...
                ->color('gray')
                ->url(fn () => route('intelligence.analysis.pdf', ['lead' => $this->lead->id, 'type' => 'geo']))
                ->openUrlInNewTab()
                ->visible(fn () => $this->lead->geoAnalysis?->status === 'completed'),

            Action::make('download_docx')
                ->label(__('leads.download_docx'))
                ->icon(Heroicon::DocumentText)
                ->color('gray')
                ->url(fn () => route('intelligence.analysis.docx', ['lead' => $this->lead->id, 'type' => 'geo']))
                ->openUrlInNewTab()
                ->visible(fn () => $this->lead->geoAnalysis?->status === 'completed'),
