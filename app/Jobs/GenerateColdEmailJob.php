...
class GenerateColdEmailJob implements ShouldQueue
{
    ...
    private function buildSystemPrompt(AiSetting $setting): string
    {
        ...
        $includes = [];
        if ($setting->include_portfolio) $includes[] = 'Mention our portfolio/past results briefly.';
        if ($setting->include_audit)     $includes[] = 'Offer a free website audit.';
        if ($setting->include_cta)       $includes[] = 'End with a clear call-to-action (book a call or reply).';
        if ($setting->include_ps)        $includes[] = 'Add a P.S. line with a bonus offer or urgency.';
