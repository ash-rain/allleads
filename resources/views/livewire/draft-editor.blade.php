<div x-data="{ tab: 'editor' }" class="flex flex-col gap-4">
    ...
    @if ($statusMessage)
        <div @class([
            ...
            'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' => $statusType === 'success',
            'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300' => $statusType === 'error',
        ])>
            {{ $statusMessage }}
        </div>
    @endif
    ...
    <div class="flex items-center justify-between gap-2">
        <div class="flex gap-2">
            <button x-on:click="tab = 'editor'"
                :class="tab === 'editor' ? 'bg-brand-blue text-white' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-300 dark:border-gray-600'"
                class="rounded-md px-3 py-1.5 text-xs font-medium transition">
                Editor
            </button>
            <button x-on:click="tab = 'refine'"
                :class="tab === 'refine' ? 'bg-brand-blue text-white' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-300 dark:border-gray-600'"
                class="rounded-md px-3 py-1.5 text-xs font-medium transition">
                AI Refine
            </button>
        </div>
    ...
    <div x-show="tab === 'editor'" class="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <div class="flex flex-col gap-1">
            ...
            <div class="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 shadow-sm h-full overflow-auto">
                @if ($subject)
                    <p class="text-xs text-gray-500 dark:text-gray-400 font-medium mb-2">Subject: {{ $subject }}</p>
                    <hr class="mb-3 border-gray-200 dark:border-gray-700">
                @endif
                <div class="text-sm text-gray-800 dark:text-gray-200 whitespace-pre-wrap leading-relaxed">{{ $body }}</div>
            </div>
        </div>
    </div>
    ...
    <div x-show="tab === 'refine'" class="flex flex-col gap-4">
        <p class="text-sm text-gray-600 dark:text-gray-400">
            Describe the change you want the AI to make to this draft. For example: <em>"Make it shorter"</em>, <em>"Add a P.S. line"</em>, <em>"Change tone to casual"</em>.
        </p>
        ...
        <p class="text-xs text-gray-500 dark:text-gray-400">
            The AI will re-generate the body. Results appear in the Editor tab after the job completes (usually &lt;30 s). Refresh the page to see the updated draft.
        </p>
    </div>
